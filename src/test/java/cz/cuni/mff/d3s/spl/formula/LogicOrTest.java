...
public class LogicOrTest extends LogicOpTestBase {
	...
	public static Collection<Object[]> createParameters() {
		return Arrays.asList(new Object[][] {
			...
			{ LogicConst.FALSE,   LogicConst.UNKNOWN, Result.CANNOT_COMPUTE },
			...
			{ LogicConst.TRUE,    LogicConst.FALSE,   Result.TRUE },
			{ LogicConst.TRUE,    LogicConst.UNKNOWN, Result.TRUE },
			{ LogicConst.TRUE,    LogicConst.TRUE,    Result.TRUE },
		});
	}
	
	public LogicOrTest(final Formula left, final Formula right, final Result result) {
		super(left, right, result);
	}
	
	@Before
	public void setupFormula() {
		constructedFormula = new LogicOr(leftSubformula, rightSubformula);
